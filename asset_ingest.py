...
def main(argv=None):
    ...
    reads 1 as "findings / proposed / applied" — so without this guard
    a crash and a real run are the same integer to gate_run and to any
    wrapper. SystemExit and KeyboardInterrupt are NOT caught: argparse
    owns exit 2 for a bad flag and must stay untouched."""
    try:
        return _main(argv)
    except Exception as err:
        report = {"tool": TOOL_NAME, "run": None,
                  "product_id": None, "counts": _new_counts(),
                  "refusals": [{"kind": "CRASH",
                                "reason": "%s: %s"
                                          % (type(err).__name__, err)}],
