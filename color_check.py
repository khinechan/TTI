...
UnicodeEncodeError on a cp1252 console (STATE.md D-378). Display only --
never touches what gets written to STATE.md/RUNLOG/JSON.
...
def _ensure_utf8_console():
    ...
    UTF-8 -- the RULE AUDIT / KNOWN GARMENTS banners below raised
    UnicodeEncodeError there (STATE.md D-378). Display only -- this never
    touches what gets written to STATE.md/RUNLOG/JSON, which stay real
    UTF-8 bytes regardless (same display-vs-stored split as gate_run.py's
    own W14 stdout_text/stderr_text decode). No-op if the stream doesn't
