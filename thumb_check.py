...
def main(argv=None):
    ...
    reads 1 as "FAIL" — a real verdict on a real image. So without
    this guard a crash and a legibility failure are the same integer
    to gate_run and to any wrapper. SystemExit and KeyboardInterrupt
    are NOT caught: argparse owns exit 2 for a bad flag and must stay
    untouched. This tool keeps no ledger, so the CRASH line IS the
    record."""
    try:
        return _main(argv)
