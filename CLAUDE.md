...
| test_play_forge.py | 30 tests: T1-T17 walls + bench F1 red→green (measured Liberation calibration in the docstring) + F5 overlap + F2 arc / F3 badge render-clean + F4 out_dir + F6 measured anchors (tall accent renders, impossible one is ELEMENT_NO_ROOM by name) + schema family/kind + config fail-closed. Renders at the real 4500×5400. unittest-style (pytest specced but absent — flagged deviation). | 1 |
...
Full suite baseline 2026-09-02: **341 tests, all green**
